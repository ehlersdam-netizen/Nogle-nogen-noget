...
choices.addEventListener("click", (event) => {
  ...
  if (Array.isArray(item.answer)) {
    // Til sætninger med flere blanks vises bare løsningen med det samme.
    allButtons.forEach((button) => {
      button.classList.add("neutral-dim");
      if (button.dataset.choice === item.answer[0] || button.dataset.choice === item.answer[1] || button.dataset.choice === item.answer[2]) {
...
newRoundBtn.addEventListener("click", () => {
  ...
  if (remainingPool.length < ROUND_SIZE) {
    remainingPool = shuffle(sentences);
  }
